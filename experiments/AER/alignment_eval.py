...
from experiments.AER.embedding import Embedding
...
sure_aligns = [[int(x) for x in l.strip().split()[:3]]
               for l in open(sys.argv[3]).readlines() if l.strip().split()[-1] != "P"]

ssents = [l.strip().split(">")[1].split("<")[0].split()
          for l in codecs.open(sys.argv[4], 'r', "utf8", errors='ignore').readlines()]
tsents = [l.strip().split(">")[1].split("<")[0].split()
          for l in codecs.open(sys.argv[5], 'r', "utf8", errors='ignore').readlines()]  # swap 3 and 4
