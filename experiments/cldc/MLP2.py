...
def get_test_doc_embedding(words, tar2src_dict, src_word_dict, src_words_idf_scores, unk):
    embedding = np.zeros([40])
    for word in words:
        word = word.lower()  # true case
        # discard the punctuations
        if word in "-,.!?:;\'\"{}[]()+-*%/=":
            continue
        if word in tar2src_dict:
            src_word = tar2src_dict[word]
            if src_word in src_word_dict:
                word_embedding = src_word_dict[src_word]
                embedding += word_embedding
        else:
            continue
            # word_embedding = word_dict[unk]
...
def get_test_data(dir_path, files, tar2src_dict, src_word_dict, src_words_idf_scores, unk="<UNK>"):
    data, labels = [], []
    for file in files:
        filename = os.path.join(dir_path, file)
        f = open(filename, "r", encoding="utf-8")
        ...
        for line in lines[1:]:
            words.extend(line.strip().split())
        embedding = get_test_doc_embedding(words, tar2src_dict, src_word_dict, src_words_idf_scores, unk)
        data.append(embedding)
    return data, labels
...
def test_mlp(src_dir_path, tar_dir_path, src_word_embeddings, src_word_idf_scores, tar2src_dict, learning_rate=0.001,
             L1_reg=0.00, L2_reg=0.0001, n_epochs=1000, batch_size=128, n_hidden=500):
    ...
    valid_x, valid_y = get_test_data(tar_dir_path, valid_files, tar2src_dict, src_word_embeddings, src_word_idf_scores,
                                     FLAGS.unk)
    test_x, test_y = get_test_data(tar_dir_path, test_files, tar2src_dict, src_word_embeddings, src_word_idf_scores,
                                   FLAGS.unk)
...
def read_most_align_words(filename):
    align_dict = {}
    f = open(filename, "r", encoding="utf-8")
    for line in f.readlines():
        lemmas = line.strip().split("|||")
...
def main(_):
    with tf.device("/gpu:0"):
        ...
        print("read align file!\n")
        word_align_dict = read_most_align_words(FLAGS.align_file)
        ...
        for index in range(10):
            ...
            results = test_mlp(FLAGS.src_dir_path, FLAGS.tar_dir_path, src_word_embedding, src_idf_scores,
                               word_align_dict)
...
if __name__ == "__main__":
    ...
    emb_path = "E:\\NLP\\code\\bilingual_word_emb\\experiment\\embedding\\en-fr"
    align_path = "E:\\NLP\\code\\bilingual_word_emb\\GIZA++"
    ...
    parse.add_argument("--tar_dir_path", type=str,
                       default=os.path.join(dir_path, "data\\cross-lingual document classification\\rcv2\\"
                                            "RCV2_sample_seg\\french_sample_seg"),
    ...
    parse.add_argument("--tar_embedding_path", type=str,
                       default=os.path.join(emb_path, "clsim\\fr.vec.txt"),
                       help="source embedding file path")
    parse.add_argument("--src_embedding_path", type=str,
                       default=os.path.join(emb_path, "clsim\\en.vec.txt"),
                       help="target embedding file path")
    parse.add_argument("--tar_idf_score_file_path", type=str,
                       default=os.path.join(dir_path, "E:\\NLP\\code\\bilingual_word_emb\\code\\sent_based_bwe\\"
                                                      "data\\en-fr\\word_idf_count\\fr.idf.score"),
                       help="source idf score file path")
    parse.add_argument("--src_idf_score_file_path", type=str,
                       default=os.path.join(dir_path, "E:\\NLP\\code\\bilingual_word_emb\\code\\sent_based_bwe\\"
                                                      "data\\en-fr\\word_idf_count\\en.idf.score"),
    ...
    parse.add_argument("--align_file", type=str, default=os.path.join(align_path, "fr2en.word.align"),
                       help="most word align file")
